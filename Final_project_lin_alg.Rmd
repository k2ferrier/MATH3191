...
moles_2 <- moles %>% 
  ...
  select(mole_count, genotype, gender, hispanic, n_vacations, haircolor, eyecolor, baseskincolor)

# PCA Regression with 8 variables
pcr <- pcr(data = moles_2, mole_count~., scale = T)
summary(pcr)
...
moles_3 <- moles %>% 
  ...
  rename("n_vacations" = number.vacs..birth.thru.2007) %>% 
  rename("mole_count" = molecount2008) %>% 
  select(mole_count, genotype, gender, eyecolor, haircolor, baseskincolor, hispanic, n_vacations)
...
moles_4 <- moles %>% 
  ...
  rename("n_vacations" = number.vacs..birth.thru.2007) %>% 
  rename("mole_count" = molecount2008) %>% 
  select(mole_count, genotype, gender, hispanic, n_vacations) 
...
Regression:
```{r}
model <- lm(data = moles_4, formula = mole_count ~ as.factor(genotype) + gender + hispanic + n_vacations)
