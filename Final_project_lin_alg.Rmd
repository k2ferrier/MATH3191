...
# Perform PCR
pcr <- pcr(data = moles_2, mole_count~., scale = T) # create a PCR object. The response variable being modeled is mole count as described by all other variables. The variables are scaled according to their variance. 
summary(pcr)
...
# Estimated coefficients (Betas) for each variable in the model based on 5 PCs
coef(pcr, comps = 5)
```
